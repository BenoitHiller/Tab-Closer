...
MatcherManager.prototype = {
  ...
  create_matcher_and_add_to_page: function() {
    this.add_matcher_to_page(this.add_matcher());
  },
  ...
  close_all_tabs: function(window_setting, window_id) {
    for (var i=0; i < this.matchers.length; i++){
      if (this.matchers[i]._regex) {
        this.matchers[i].find_all_tabs(window_setting,window_id);
      }
      else {
        this.matchers[i].find_tabs(window_setting,window_id);
      }
    }
  },
